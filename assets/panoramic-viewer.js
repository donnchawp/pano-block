class PanoramicViewer {
    ...
    createModal() {
        ...
        // Bind modal events
        this.modal.querySelector('.pano-close').addEventListener('click', () => this.close());
        this.modal.querySelector('.pano-zoom-in').addEventListener('click', () => this.zoom(1.2));
        this.modal.querySelector('.pano-zoom-out').addEventListener('click', () => this.zoom(0.8));
        this.modal.querySelector('.pano-zoom-reset').addEventListener('click', () => this.resetView());
        ...
        // Touch events
        this.viewer.addEventListener('touchstart', (e) => this.startTouch(e));
        this.viewer.addEventListener('touchmove', (e) => this.dragTouch(e));
        this.viewer.addEventListener('touchend', () => this.endDrag());
        
        // Keyboard events
        this.viewer.addEventListener('keydown', (e) => this.handleKeydown(e));
    ...
    bindEvents() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            thumbnails.forEach(thumbnail => {
                thumbnail.addEventListener('click', (e) => this.openViewer(e.currentTarget));
    ...
    async generateThumbnail(thumbnail) {
        ...
        try {
            ...
            const scale = Math.min(maxThumbnailWidth / totalWidth, maxThumbnailHeight / maxHeight, 1);
            ...
            images.forEach((img, index) => {
                ...
                ctx.drawImage(img, x - overlap, y, scaledWidth + overlap, scaledHeight);
                x += scaledWidth;
            });
            
            // Replace the placeholder image with stitched image
            const stitchedImg = thumbnail.querySelector('.pano-stitched-image');
            if (stitchedImg) {
    ...
    async loadImages(imagesData) {
        this.images = await Promise.all(imagesData.map(imgData => {
            return new Promise((resolve, reject) => {
                const img = new Image();
                img.crossOrigin = 'anonymous';
                img.onload = () => resolve(img);
    ...
    async stitchImages() {
        ...
        this.images.forEach((img, index) => {
            ...
            // Add slight overlap except for first image
            const overlap = index > 0 ? 2 : 0;
            stitchedCtx.drawImage(img, x - overlap, y, img.width + overlap, img.height);
            x += img.width;
        });
    }
    ...
    render() {
        ...
        this.ctx.drawImage(
            this.stitchedCanvas,
            0, 0, this.stitchedCanvas.width, this.stitchedCanvas.height,
